...
def compute_volumes_and_save_csv(config):
    ...
    for seg_file in sorted(LABELS_DIR.glob("*.nii.gz")):
        case_id = seg_file.stem
        volume_mm3 = whole_tumor_volume_mm3(str(seg_file))
        volume_cm3 = volume_mm3 / 1000.0
        records.append({
            "case_id": case_id,
            "wt_volume_mm3": float(volume_mm3),
            "wt_volume_cm3": float(volume_cm3)
        })
...
def plot_error_histogram(errors, title='Histogram of Prediction Errors _ 2', save_path="../output/plot_error_histogram.png"):
    plt.figure(figsize=(8, 6))
...
def plot_bland_altman(true_y, pred_y, title='Bland-Altman Plot _ 2', save_path="../output/plot_bland_altman.png"):
...
if __name__ == "__main__":
    ...
    for fold in [2]:  # Example fold
        trainer_fold, _ = load_model_and_checkpoint_files(config['nnunet_fold_folder'], folds=[fold], checkpoint_name="model_final_checkpoint")
        ...
        if not config['train_backbone']:
            set_requires_grad(model_fold.backbone, False)
        if config['freeze_seg_outputs'] and hasattr(model_fold.backbone, "seg_outputs"):
            set_requires_grad(model_fold.backbone.seg_outputs, False)
        set_requires_grad(model_fold.head, True)
    ...
    optim = torch.optim.AdamW([p for m in models for p in filter(lambda param: param.requires_grad, m.parameters())],
    ...
    with open("../output/log.txt", "w") as log_file:
        for epoch in range(config['epochs']):
            ...
            for xb, yb in tqdm(dl_tr, desc=f"Epoch {epoch+1}/{config['epochs']} [Train]"):
                ...
                optim.zero_grad()
                pred_norms = [m(xb) for m in models]
                pred_norm_avg = torch.mean(torch.stack(pred_norms), dim=0)
                loss = loss_fn(pred_norm_avg, yb)
                loss.backward()
                optim.step()
            ...
            with torch.no_grad():
                for xb, yb in tqdm(dl_val, desc=f"Epoch {epoch+1}/{config['epochs']} [Val]"):
                    ...
                    pred_norms = [m(xb) for m in models]
                    pred_norm_avg = torch.mean(torch.stack(pred_norms), dim=0)
                    pred = np.expm1(pred_norm_avg.item() * y_std_log + y_mean_log)
            ...
            if val_mae < best_val_mae:
                ...
                for fold_idx, m in enumerate(models):
                    torch.save(m.head.state_dict(), f"../output/best_scalar_head_fold{fold_idx}.pth")
                    torch.save(m.state_dict(), f"../output/full_model{fold_idx}.pth")
